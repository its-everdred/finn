...
scene("battle", (which, bopts = {}) => {
  ...
  function confirm() {
    ...
    if (sub === "item") {
      ...
      if (subIdx === 0) {
        ...
        sub = null; busy = true; say([`* ${cur.label} ate a Cookie. +${mhp(cur) - was} HP!`], actionDone); return;
      }
      if (state.juice <= 0) { music.sfx("back"); say(["* No juice left!"]); return; }
      music.sfx("heal");
      state.juice -= 1; const was = mhp(cur); setHp(cur, mmax(cur));
      sub = null; busy = true; say([`* ${cur.label} drank the Juice Box. +${mhp(cur) - was} HP! Full health!`], actionDone); return;
    }
  ...
  function defend(f, rolled, done) {
    ...
    if (d.defend === "mashB") {
      ...
      telegraph(`${f.name} ATTACKS!`, () =>
        miniCue("timing", C_BLUE, () => miniTiming({ prompt: "BLOCK! TAP A!", keys: INTERACT.concat(BACK), speed, zone: f.mini.zone || 0.3, zones: d.zones || 1, hot: C_BLUE }, (g) => land(g === "perfect" ? 0.7 : g === "good" ? 0.4 : 0))));
    }
  }
